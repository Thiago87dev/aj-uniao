    ...
    <section id='contato' aria-labelledby='contato-titulo' className='bg-[#141c26] px-5 py-16 text-white sm:px-10 lg:px-16 lg:py-24 xl:px-40'>
      ...
      <div className='mt-10 grid gap-8 lg:grid-cols-[0.8fr_1.2fr]'>
        <div className='min-w-0 space-y-7'>
          ...
          <div><h3 className='font-bold text-secondary'>Fale com a equipe</h3><p className='mt-2 text-sm text-slate-300'>{oficina.telefone}</p>{oficina.whatsapp ? <Button variant='custom' fontSize='sm' uppercase={false} href={`https://wa.me/${oficina.whatsapp}`} target='_blank' rel='noopener noreferrer' className='mt-4 inline-block rounded-xl bg-secondary px-5 py-3 font-black text-primary'>Conversar no WhatsApp</Button> : <p className='mt-2 text-xs text-slate-400'>Telefone ilustrativo. Contato real em breve.</p>}</div>
        </div>
        <div className='min-w-0 overflow-hidden rounded-xl border border-white/15 bg-[#0b1015]'>
          ...
          <Button variant='custom' fontSize='sm' uppercase={false} href={`https://www.google.com/maps/dir/?api=1&destination=${consulta}&travelmode=driving`} target='_blank' rel='noopener noreferrer' className='flex items-center justify-between gap-3 px-5 py-4 font-semibold text-secondary'>Como chegar<FaArrowUpRightFromSquare aria-hidden='true' className='shrink-0' /></Button>
        </div>
      </div>
    </section>
  );
